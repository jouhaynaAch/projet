...
def edit(id):
    ...
    if request.method=="POST":
        ...
        query=db.engine.execute(f"UPDATE `student` SET `rollno`='{rollno}',`sname`='{sname}',`sem`='{sem}',`gender`='{gender}',`branch`='{branch}',`email`='{email}',`note`='{note}'")
        flash("Student is Updates","success")
        return redirect('/studentdetails')
...
def addstudent():
    ...
    if request.method=="POST":
        ...
        email=request.form.get('email')
        note=request.form.get('note')
        query=db.engine.execute(f"INSERT INTO `student` (`rollno`,`sname`,`sem`,`gender`,`branch`,`email`,`note`) VALUES ('{rollno}','{sname}','{sem}','{gender}','{branch}','{email}','{note}')")
